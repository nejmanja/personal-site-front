...
export async function getPageCount(db, category) {
    // get amount of entries for category, if such exists
    const postCnt = await db
        .collection("posts")
        .countDocuments(category && { categories: category });

    // round to number of pages
    return Math.ceil(postCnt / PER_PAGE);
}

// get paths for post indexes for each category
...
// get page props for post index pages
